...
def get_epistasis_stats(landscape: potts_model.PottsModel, threshold: float = 0, adaptive: bool = True) -> Tuple[float, float]:
...
def get_landscape_stats(landscape: potts_model.PottsModel) -> dict:
  ...
  reciprocal_adaptive_epistasis_effect, fraction_reciprocal_adaptive_epistasis = get_epistasis_stats(landscape, threshold=0.0, adaptive=True)
...
def get_coupling_scale(landscape: potts_model.PottsModel,
  ...
  mean_adaptive_single_effect = get_mean_single_effect(landscape, threshold=adaptive_threshold, adaptive=is_adaptive) + single_mut_offset
  untuned_epistasis_effect = get_doubles_df(landscape, threshold=adaptive_threshold, adaptive=is_adaptive).residual.mean()
...
# TODO(nthomas) fraction_reciprocal_deleterious_epistasis: The fraction of deleterious(-, -) doubles that exhibit positive epistasis.
def get_tuning_parameters(landscape: potts_model.PottsModel,
  ...
  return {'coupling_scale': coupling_scale, 'field_scale': field_scale, 'single_mut_offset': single_mut_offset, 'epi_offset': epi_offset}
